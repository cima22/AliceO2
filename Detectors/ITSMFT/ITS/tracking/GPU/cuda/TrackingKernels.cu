  ...
  float* linePt)
{
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < nCells; i += blockDim.x * gridDim.x) {
    ...
    const auto sym = cell.getTimeStamp().makeSymmetrical();
    lineTimes[slot] = LineTime{sym.getTimeStamp(), sym.getTimeStampError()};
...
struct MemberCount {
  ...
  GPUhdi() int operator()(const int p) const { return cands[p].keep ? cands[p].size : 0; }
};

template <int NLayers>
GPUg() void collectLinesForMCKernel(const int* nPeaksDevice,
                                    const int* peakLineIdx,
                                    const gpu::LineWindow* win,
                                    const LineProjSoA lineProjSorted,
                                    const GPULine* lines,
...
template <int NLayers>
void registerClusterOwnershipHandler(const CellSeed* cells,
                                     const int nCells,
...
template <int NLayers>
void linearizeCellsToLinesHandler(const int nCells,
                                  const CellSeed* cells,
                                  const unsigned long long* const* clusterOwners,
...
template <int NLayers>
void sortLinesHandler(const int nLines,
                      const int nRofs,
...
template <int NLayers>
void scanDensityHandler(const int nLines,
                        const gpu::LineProjSoA sortedSoa,
...
template <int NLayers>
void findPeaksHandler(const int nLines,
                      const int nRofs,
                      const gpu::LineProjSoA sortedSoa,
                      const int* rofOffsets,
                      const int* density,
...
template <int NLayers>
void fitPeaksHandler(const int* nPeaksDevice,
                     const int* peakLineIdx,
                     const gpu::LineWindow* win,
                     const gpu::LineProjSoA sortedSoa,
...
template <int NLayers>
void dedupVertexCandidatesHandler(const int* nPeaksDevice,
                                  const int* peakLineIdx,
                                  const int* peakOffsets,
                                  const gpu::LineProjSoA sortedSoa,
                                  const float duplicateZCut,
...
// MC only
template <int NLayers>
void scanMemberOffsetsHandler(const gpu::VertexCand* cands,
                              int* memberOffsets,
                              const int nLines,
                              o2::its::ExternalAllocator* alloc,
                              gpu::Stream& stream)
{
  auto nosync_policy = THRUST_NAMESPACE::par_nosync(gpu::TypedAllocator<char>(alloc)).on(stream.get());
  auto memberCount = thrust::make_transform_iterator(thrust::make_counting_iterator(0), gpu::MemberCount{cands});
  thrust::exclusive_scan(nosync_policy, memberCount, memberCount + nLines + 1, memberOffsets, 0, thrust::plus<int>());
}

// MC only
template <int NLayers>
void collectLinesForMCHandler(const int* nPeaksDevice,
                              const int* peakLineIdx,
                              const gpu::LineWindow* win,
                              const gpu::LineProjSoA sortedSoa,
                              const gpu::GPULine* lines,
                              const float pairCut2,
                              const gpu::VertexCand* cands,
                              const int* memberOffsets,
                              int* memberLines,
                              gpu::Stream& stream)
{
...
template void registerClusterOwnershipHandler<7>(const CellSeed* cells,
                                                 const int nCells,
                                                 unsigned long long** clusterOwnersDeviceArray,
                                                 gpu::Stream& stream);

template void sortClustersHandler<7>(const Cluster* unsorted, Cluster* sorted, const int* clusterOffsets,
                                     int* indexTable, const IndexTableUtils<7>* utils,
                                     const typename ROFMaskTable<7>::View& rofMask, float beamX, float beamY,
                                     int zBins, int phiBins, int nRofs, int nClustersLayer, int iLayer,
                                     float* minRadiusLayer, float* maxRadiusLayer, int* keys, int* perm,
                                     o2::its::ExternalAllocator* alloc, gpu::Stream& stream);

template void linearizeCellsToLinesHandler<7>(const int nCells,
                                              const CellSeed* cells,
                                              const unsigned long long* const* clusterOwners,
                                              const int* rofFramesClustersL1,
                                              const int nRofsL1,
...
template void sortLinesHandler<7>(const int nLines,
                                  const int nRofs,
                                  const gpu::LineProjSoA soa,
                                  const gpu::LineProjSoA sortedSoa,
                                  const int* lineRof,
...
template void scanDensityHandler<7>(const int nLines,
                                    const gpu::LineProjSoA sortedSoa,
                                    const int* rofOffsets,
                                    int* density,
                                    gpu::LineWindow* win,
...
template void findPeaksHandler<7>(const int nLines,
                                  const int nRofs,
                                  const gpu::LineProjSoA sortedSoa,
                                  const int* rofOffsets,
                                  const int* density,
...
template void fitPeaksHandler<7>(const int* nPeaksDevice,
                                 const int* peakLineIdx,
                                 const gpu::LineWindow* win,
                                 const gpu::LineProjSoA sortedSoa,
                                 const gpu::GPULine* lines,
...
template void dedupVertexCandidatesHandler<7>(const int* nPeaksDevice,
                                              const int* peakLineIdx,
                                              const int* peakOffsets,
                                              const gpu::LineProjSoA sortedSoa,
                                              const float duplicateZCut,
...
template void scanMemberOffsetsHandler<7>(const gpu::VertexCand* cands,
                                          int* memberOffsets,
                                          const int nLines,
                                          o2::its::ExternalAllocator* alloc,
                                          gpu::Stream& stream);

template void collectLinesForMCHandler<7>(const int* nPeaksDevice,
                                          const int* peakLineIdx,
                                          const gpu::LineWindow* win,
                                          const gpu::LineProjSoA sortedSoa,
                                          const gpu::GPULine* lines,
...
#ifdef ENABLE_UPGRADES
template void registerClusterOwnershipHandler<11>(const CellSeed* cells,
                                                  const int nCells,
                                                  unsigned long long** clusterOwnersDeviceArray,
                                                  gpu::Stream& stream);

template void sortClustersHandler<11>(const Cluster* unsorted, Cluster* sorted, const int* clusterOffsets,
                                      int* indexTable, const IndexTableUtils<11>* utils,
                                      const typename ROFMaskTable<11>::View& rofMask, float beamX, float beamY,
                                      int zBins, int phiBins, int nRofs, int nClustersLayer, int iLayer,
                                      float* minRadiusLayer, float* maxRadiusLayer, int* keys, int* perm,
                                      o2::its::ExternalAllocator* alloc, gpu::Stream& stream);
...
template void scanDensityHandler<11>(const int nLines,
                                     const gpu::LineProjSoA sortedSoa,
                                     const int* rofOffsets,
                                     int* density,
                                     gpu::LineWindow* win,
...
template void findPeaksHandler<11>(const int nLines,
                                   const int nRofs,
                                   const gpu::LineProjSoA sortedSoa,
                                   const int* rofOffsets,
                                   const int* density,
...
template void fitPeaksHandler<11>(const int* nPeaksDevice,
                                  const int* peakLineIdx,
                                  const gpu::LineWindow* win,
                                  const gpu::LineProjSoA sortedSoa,
                                  const gpu::GPULine* lines,
...
template void dedupVertexCandidatesHandler<11>(const int* nPeaksDevice,
                                               const int* peakLineIdx,
                                               const int* peakOffsets,
                                               const gpu::LineProjSoA sortedSoa,
                                               const float duplicateZCut,
...
template void scanMemberOffsetsHandler<11>(const gpu::VertexCand* cands,
                                           int* memberOffsets,
                                           const int nLines,
                                           o2::its::ExternalAllocator* alloc,
                                           gpu::Stream& stream);

template void collectLinesForMCHandler<11>(const int* nPeaksDevice,
                                           const int* peakLineIdx,
                                           const gpu::LineWindow* win,
                                           const gpu::LineProjSoA sortedSoa,
                                           const gpu::GPULine* lines,
...
template void registerClusterOwnershipHandler<13>(const CellSeed* cells,
                                                  const int nCells,
                                                  unsigned long long** clusterOwnersDeviceArray,
                                                  gpu::Stream& stream);

template void sortClustersHandler<13>(const Cluster* unsorted, Cluster* sorted, const int* clusterOffsets,
                                      int* indexTable, const IndexTableUtils<13>* utils,
                                      const typename ROFMaskTable<13>::View& rofMask, float beamX, float beamY,
                                      int zBins, int phiBins, int nRofs, int nClustersLayer, int iLayer,
                                      float* minRadiusLayer, float* maxRadiusLayer, int* keys, int* perm,
                                      o2::its::ExternalAllocator* alloc, gpu::Stream& stream);
...
template void scanDensityHandler<13>(const int nLines,
                                     const gpu::LineProjSoA sortedSoa,
                                     const int* rofOffsets,
                                     int* density,
                                     gpu::LineWindow* win,
...
template void findPeaksHandler<13>(const int nLines,
                                   const int nRofs,
                                   const gpu::LineProjSoA sortedSoa,
                                   const int* rofOffsets,
                                   const int* density,
...
template void fitPeaksHandler<13>(const int* nPeaksDevice,
                                  const int* peakLineIdx,
                                  const gpu::LineWindow* win,
                                  const gpu::LineProjSoA sortedSoa,
                                  const gpu::GPULine* lines,
...
template void dedupVertexCandidatesHandler<13>(const int* nPeaksDevice,
                                               const int* peakLineIdx,
                                               const int* peakOffsets,
                                               const gpu::LineProjSoA sortedSoa,
                                               const float duplicateZCut,
...
template void scanMemberOffsetsHandler<13>(const gpu::VertexCand* cands,
                                           int* memberOffsets,
                                           const int nLines,
                                           o2::its::ExternalAllocator* alloc,
                                           gpu::Stream& stream);

template void collectLinesForMCHandler<13>(const int* nPeaksDevice,
                                           const int* peakLineIdx,
                                           const gpu::LineWindow* win,
                                           const gpu::LineProjSoA sortedSoa,
                                           const gpu::GPULine* lines,
