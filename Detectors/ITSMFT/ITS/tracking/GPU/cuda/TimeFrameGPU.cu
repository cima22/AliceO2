...
void TimeFrameGPU<NLayers>::sortClustersDevice(const int layer, const TrackingParameters& trkParam)
{
  ...
  sortClustersHandler<NLayers>(mUnsortedClustersDevice[layer], mClustersDevice[layer],
...
void TimeFrameGPU<NLayers>::createLinesDevice(const int nCells)
{
  ...
  mLineTimesDevice = allocDeviceAsync<gpu::LineTime>(nCells, mGpuStreams[0], kStack);
  mLineZsSortedDevice = allocDeviceAsync<float>(nCells, mGpuStreams[0], kStack);
  mLineTimesSortedDevice = allocDeviceAsync<gpu::LineTime>(nCells, mGpuStreams[0], kStack);
  ...
  mMemberOffsetsDevice = allocDeviceAsync<int>((nCells + 1), mGpuStreams[0], kStack);
...
void TimeFrameGPU<NLayers>::wipe()
{
  ...
  mPersistentTablesLoaded = false;
  mUnsortedClustersUploaded.reset();
  mTrackingFrameInfoUploaded.reset();
}
