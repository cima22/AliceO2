...
void TrackerTraits<NLayers>::computeLayerTracklets(const int iteration, int iVertex)
{
  ...
  mTaskArena->execute([&] {
    ...
    auto forTracklets = [&](int linkId, int pivotROF, auto&& emit) {
      ...
      const float phiCut = vtxMode ? o2::its::VertexerParamConfig::Instance().phiCut : mTimeFrame->getLinkPhiCut(linkId);
...
void TrackerTraits<NLayers>::computeVertexCandidates(const int iteration)
{
  ...
  const auto& vtxConf = o2::its::VertexerParamConfig::Instance();
  const float maxZ = vtxConf.maxZPositionAllowed;
  const float lineMinPt = vtxConf.lineMinPt;
  ...
  for (int k = 0; k < nCells; ++k) {
    ...
    mTimeFrame->getLinesQuality(pivotRof).emplace_back(
      typename TimeFrame<NLayers>::LineQuality{cells[k].getChi2(), cells[k].getPt()});
    if (withMC) {
...
namespace
{
// Majority MC label of a set of line labels (ported from VertexerTraits): we only care about the
// source&event of the contributing tracks, not their trackId; flag as fake if no label has a strict
// majority (>50%).
VertexLabel computeMain(const bounded_vector<o2::MCCompLabel>& elements)
{
...
bool TrackerTraits<NLayers>::skipROF(int iteration, int rof) const
{
  return mTrkParams[iteration].PassFlags[IterationStep::SkipROFsAboveThreshold] &&
         (int)mTimeFrame->getROFVertexLookupTableView().getVertices(1, rof).getEntries() >
           o2::its::VertexerParamConfig::Instance().vertPerRofThreshold;
}
...
void TrackerTraits<NLayers>::computeBeamFromVertices(const int)
{
  ...
  LOGP(info, "[beamFit] beam from {} vertices = ({:.4f}, {:.4f}) cm  (sum nContrib={:.0f})", vertices.size(), bx, by, sw);
}
...
void TrackerTraits<NLayers>::computeVertices(const int iteration)
{
  ...
  const float maxZ = vc.maxZPositionAllowed;
  const float clusterCut = vc.clusterCut;
  const float pairCut2 = vc.pairCut * vc.pairCut;
  const float zWindow = 0.5f * clusterCut;
  const float nSigmaCut = vc.nSigmaCut;
  const float duplicateZCut = vc.duplicateZCut > 0.f
                                ? vc.duplicateZCut
                                : std::max(4.f * vc.pairCut, 0.5f * clusterCut);
  const int minContributors = vc.clusterContributorsCut;
  const int suppressLowMultDebris = vc.suppressLowMultDebris;
  const bool incremental = vc.incrementalSeeding;
  const float fineZWindow = vc.fineZWindow;
  const bool doFine = fineZWindow > 0.f && fineZWindow < zWindow;
  const int fineMinDensity = vc.fineMinDensity;
  const float fineMaxDrift = vc.fineMaxDrift;
  const float goodSig = vc.goodContributorsSignificance;
  const float duplicateZScale = vc.duplicateZScale;
  const float goodLineChi2Cut = vc.goodLineChi2Cut;
  const float goodLinePtCut = std::abs(mBz) > 0.01f ? vc.goodLinePtCut : -1.f;
  ...
  const auto processROF = [&](const int rofId) {
    ...
    bounded_vector<LineTime> tc(mMemoryPool.get());
    ...
    for (int i = 0; i < n; ++i) {
      ...
      const auto sym = line.mTime.makeSymmetrical();
      zc.push_back(z);
      tc.push_back(LineTime{sym.getTimeStamp(), sym.getTimeStampError()});
      lic.push_back(i);
    }
    ...
    bounded_vector<LineTime> T(m, mMemoryPool.get());
    ...
    computeDensities(Z, T, zWindow, incremental, win, density);
    bounded_vector<int> densityFine(doFine ? m : 0, mMemoryPool.get());
    bounded_vector<LineWindow> winFine(doFine ? m : 0, mMemoryPool.get());
    if (doFine) {
      computeDensities(Z, T, fineZWindow, incremental, winFine, densityFine);
    }

    // peaks: leftmost density maxima
    bounded_vector<uint8_t> isMax(m, mMemoryPool.get());
    markLeftmostMaxima(density, Z, win, incremental, mMemoryPool.get(), isMax);
    bounded_vector<uint8_t> isMaxFine(doFine ? m : 0, mMemoryPool.get());
    if (doFine) {
      markLeftmostMaxima(densityFine, Z, winFine, incremental, mMemoryPool.get(), isMaxFine);
    }
    ...
    for (const int p : accepted) {
      ...
      if (withMC) {
        ...
        for (const auto idx : cand[p].getLabels()) {
          if (idx < 0 || idx >= nLab) {
            LOGP(error, "[seedDbg] OOB lineLabel idx={} nLab={} rof={}", idx, nLab, rofId);
            continue;
          }
    ...
    auto forTrackletCells = [&](int cellTopologyId, int iTracklet, auto&& emit) {
      ...
      const float cellDeltaPhiCut =
        mTrkParams[iteration].PassFlags[IterationStep::SeedingVertexPass]
          ? cellDeltaPhiBound(mBz, mTrkParams[iteration].CellDeltaPhiMinPt,
      ...
      for (int iNextTracklet{nextLayerFirstTrackletIndex}; iNextTracklet < nextLayerLastTrackletIndex; ++iNextTracklet) {
        ...
        if (cellDeltaPhiCut > 0.f) {
          float deltaPhi{std::abs(currentTracklet.phi - nextTracklet.phi)};
          if (deltaPhi > o2::constants::math::PI) { // the two directions straddle the +-pi wrap
